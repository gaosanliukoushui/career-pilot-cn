...
export function CandidateStructureEditor() {
  ...
  function setEducation(key: string, value: unknown, factId: string) {
    setStructured((current) => ({ ...current, education: { ...current.education, [key]: value === "" || !factId ? undefined as never : { value, fact_id: factId } } }));
  }

  function setPreference(key: string, value: unknown, factId: string) {
    setStructured((current) => ({ ...current, preferences: { ...current.preferences, [key]: value === "" || !factId ? undefined as never : { value, fact_id: factId } } }));
  }

  function setLanguage(next: Partial<Structured["language_certificates"][number]>) {
    setStructured((current) => {
      const existing = current.language_certificates[0];
      const value = { kind: next.kind ?? existing?.kind ?? "CET4", fact_id: next.fact_id ?? existing?.fact_id ?? "", level: next.level ?? existing?.level, score: next.score ?? existing?.score };
      return { ...current, language_certificates: value.fact_id ? [value] : [] };
    });
  }

  function setCredential(next: Partial<Structured["credentials"][number]>) {
    setStructured((current) => {
      const existing = current.credentials[0];
      const value = { name: next.name ?? existing?.name ?? "", fact_id: next.fact_id ?? existing?.fact_id ?? "", level: next.level ?? existing?.level };
      return { ...current, credentials: value.name && value.fact_id ? [value] : [] };
    });
  }
  ...
  async function save() {
    setBusy(true); setMessage("");
    const response = await fetch("/api/cn/profile/structured", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ structured }) });
    const data = await response.json();
    if (response.ok) { setProfile(data); setMessage("结构化资格资料已保存，引用事实已授权用于岗位匹配和网申草稿"); }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-5 md:p-8">
      ...
      <section className="space-y-4 rounded-xl border border-border bg-surface p-5">
        <div><h2 className="text-lg font-semibold">第二步：补充证据并确认</h2><p className="mt-1 text-sm text-muted">普通偏好可由本人确认；学历、成绩、证书、经历和成果必须关联可核验的 HTTPS 强证据。</p></div>
        <div className="space-y-3">
          {(profile.facts || []).map((fact) => {
            ...
            return <article key={fact.id} className="rounded-lg border border-border bg-background/60 p-4">
              ...
              <p className="mt-2 text-xs text-faint">证据：{evidence.length ? evidence.map((item) => `${item.strength === "strong" ? "强" : "普通"}/${item.kind}`).join("、") : "未关联"}</p>
      ...
      <section className="space-y-4 rounded-xl border border-border bg-surface p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {educationRows.map((row) => {
            ...
            return <div key={row.key} className="rounded-lg border border-border p-3"><label className="text-sm font-medium">{row.label}</label>{row.type === "select" ? <select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={String(current?.value ?? "")} onChange={(event) => setEducation(row.key, event.target.value, current?.fact_id || "")}><option value="">待填写</option>{row.options?.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : <input type={row.type} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={String(current?.value ?? "")} onChange={(event) => setEducation(row.key, row.type === "number" ? Number(event.target.value) : event.target.value, current?.fact_id || "")} />}<select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm" value={current?.fact_id || ""} onChange={(event) => setEducation(row.key, current?.value ?? "", event.target.value)}><option value="">选择证据事实</option>{factOptions(["education", "grade", "ranking", "certificate"]).map((fact) => <option key={fact.id} value={fact.id}>{fact.statement} · {fact.id}</option>)}</select></div>;
          })}
        </div>
      </section>
      <section className="space-y-4 rounded-xl border border-border bg-surface p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-border p-3">
            <label className="text-sm font-medium">英语等级</label>
            <div className="mt-2 grid grid-cols-2 gap-2"><select className="rounded-md border border-border bg-background px-3 py-2" value={structured.language_certificates[0]?.kind || "CET4"} onChange={(event) => setLanguage({ kind: event.target.value })}><option>CET4</option><option>CET6</option></select><input type="number" className="rounded-md border border-border bg-background px-3 py-2" value={structured.language_certificates[0]?.score ?? ""} onChange={(event) => setLanguage({ score: event.target.value ? Number(event.target.value) : undefined })} placeholder="成绩" /></div>
            <select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm" value={structured.language_certificates[0]?.fact_id || ""} onChange={(event) => setLanguage({ fact_id: event.target.value })}><option value="">选择证书事实</option>{factOptions(["certificate"]).map((fact) => <option key={fact.id} value={fact.id}>{fact.statement}</option>)}</select>
          </div>
          <div className="rounded-lg border border-border p-3">
            <label className="text-sm font-medium">其他资格证书</label><input className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={structured.credentials[0]?.name || ""} onChange={(event) => setCredential({ name: event.target.value })} placeholder="证书名称" /><select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm" value={structured.credentials[0]?.fact_id || ""} onChange={(event) => setCredential({ fact_id: event.target.value })}><option value="">选择证书事实</option>{factOptions(["certificate"]).map((fact) => <option key={fact.id} value={fact.id}>{fact.statement}</option>)}</select>
          </div>
      ...
      <section className="space-y-4 rounded-xl border border-border bg-surface p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-border p-3"><label className="text-sm font-medium">意向城市</label><input className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={Array.isArray(structured.preferences.locations?.value) ? (structured.preferences.locations.value as string[]).join("、") : ""} onChange={(event) => setPreference("locations", event.target.value.split(/[、，,]/).map((item) => item.trim()).filter(Boolean), structured.preferences.locations?.fact_id || "")} placeholder="北京、上海" /><select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={structured.preferences.locations?.fact_id || ""} onChange={(event) => setPreference("locations", structured.preferences.locations?.value || [], event.target.value)}><option value="">选择偏好事实</option>{factOptions(["preference", "basic"]).map((fact) => <option key={fact.id} value={fact.id}>{fact.statement}</option>)}</select></div>
          {[["relocation", "接受异地"], ["rotation", "接受轮岗"], ["grassroots_service", "接受基层"], ["travel", "接受出差"], ["adjustment", "接受调剂"]].map(([key, label]) => <div key={key} className="rounded-lg border border-border p-3"><label className="text-sm font-medium">{label}</label><select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={String(structured.preferences[key]?.value || "")} onChange={(event) => setPreference(key, event.target.value, structured.preferences[key]?.fact_id || "")}><option value="">待确认</option><option value="yes">是</option><option value="no">否</option><option value="unknown">视情况</option></select><select className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2" value={structured.preferences[key]?.fact_id || ""} onChange={(event) => setPreference(key, structured.preferences[key]?.value || "", event.target.value)}><option value="">选择偏好事实</option>{factOptions(["preference"]).map((fact) => <option key={fact.id} value={fact.id}>{fact.statement}</option>)}</select></div>)}
        </div>
      </section>
