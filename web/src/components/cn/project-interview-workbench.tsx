...
export function ProjectInterviewWorkbench() {
  ...
  async function reviewAnswer() {
    ...
    try {
      ...
      setReview(await jsonBody<ProjectInterviewReview>(response));
    } catch (reason) {
    ...
    <div className="space-y-7">
      <section className="rounded-2xl border border-border bg-surface p-5 md:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:min-w-[36rem]">
            ...
            <label className="text-sm"><span className="mb-1.5 block text-muted">目标岗位</span><input aria-label="目标岗位" value={targetRole} onChange={(event) => setTargetRole(event.target.value)} maxLength={120} className="w-full rounded-lg border border-border bg-background px-3 py-2.5 outline-none focus:border-brand/60" placeholder="如：AI 应用开发工程师" /></label>
          </div>
        </div>
      </section>
      ...
      {project && <section className="rounded-2xl border border-border bg-surface p-5 md:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl"><div className="flex items-center gap-2 text-brand-text"><Target className="size-4" /><span className="text-xs font-semibold uppercase tracking-[0.16em]">当前训练项目</span></div><h2 className="mt-2 text-2xl font-semibold">{project.name}</h2><p className="mt-2 text-sm leading-6 text-muted">{project.summary}</p><div className="mt-3"><FactBadges ids={project.fact_ids} /></div></div>
          <div className="flex min-w-64 flex-col gap-2"><label className="text-sm"><span className="mb-1.5 block text-muted">只读 AI 模型</span><select aria-label="只读 AI 模型" value={cliId} onChange={(event) => setCliId(event.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2.5"><option value="">没有满足安全策略的 CLI</option>{clis.map((cli) => <option key={cli.id} value={cli.id}>{cli.name}</option>)}</select></label><button type="button" onClick={generatePack} disabled={!cliId || busy !== ""} className="inline-flex items-center justify-center rounded-lg bg-brand px-4 py-2.5 text-sm font-medium text-brand-foreground disabled:opacity-50">{busy === "pack" ? <><LoaderCircle className="mr-2 size-4 animate-spin" />{cliName} 正在生成训练包…</> : pack ? <><RefreshCw className="mr-2 size-4" />重新生成训练包</> : <><Sparkles className="mr-2 size-4" />生成分析、题目与参考答案</>}</button>{!cliId && <Link href="/config" className="text-center text-xs text-brand-text hover:underline">先配置 Claude Code 或 Codex</Link>}</div>
        </div>
      </section>}
      ...
      {pack && <>
        ...
        <section id="mock-interview" className="scroll-mt-6 rounded-2xl border border-border bg-surface p-5 md:p-6" aria-labelledby="mock-interview-title">
          ...
          <label className="mt-4 block"><span className="mb-2 block text-sm font-medium">你的回答</span><textarea aria-label="你的项目面试回答" value={answer} onChange={(event) => setAnswer(event.target.value)} maxLength={6000} rows={7} placeholder="建议按：场景 → 本人责任 → 方案与取舍 → 验证 → 当前边界" className="w-full resize-y rounded-xl border border-border bg-background p-4 text-sm leading-7 outline-none focus:border-brand/60" /></label>
