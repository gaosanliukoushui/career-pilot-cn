...
export function ApplyView() {
      ...
      <div>
        <div className="flex max-w-2xl items-center gap-2 rounded-full border border-border bg-surface/70 py-1.5 pl-4 pr-1.5 shadow-sm transition focus-within:border-brand/50 focus-within:shadow-md">
          <input
            ...
            placeholder="粘贴申请表链接（Ashby、Lever、Greenhouse 等）"
              ...
              <div className="min-w-0">
                ...
                {a.url && /^https?:\/\//.test(a.url) && (
                  <a href={a.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-brand hover:underline">
                    直接打开真实表单 <ExternalLink className="size-3" />
                  </a>
                )}
              </div>
            </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {opening && (
        <>
          <ProcessingHero title="正在读取申请表…" subtitle="正在本机打开真实申请页面并读取每个字段。" />
          <FieldSkeleton />
        </>
      )}
        ...
        <div className="co-rise">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {prefilling ? "正在根据简历起草…" : "根据我的简历预填"}
            </button>
            <span className="text-xs text-muted">……也可以让右下角助手撰写或修改任意回答。</span>
          </div>
          ...
          {(prefilling || a.prefillLog.length > 0) && (
            <details className="mb-4 rounded-lg border border-border bg-surface/60 backdrop-blur-sm" open={false}>
              <summary className="flex cursor-pointer select-none items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-muted">
                ...
                <span className="ml-auto text-faint">{a.prefillLog.length} 个步骤</span>
              </summary>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            >
              ...
              {filling ? "正在填写真实表单…" : "填写真实表单并审核"}
            </button>
            <button
              onClick={a.agentFill}
              disabled={filling || prefilling}
              title="让 AI 在真实表单中逐项填写，适合复杂或多步骤表单；系统不会自动提交。"
            ...
            <p className="inline-flex items-center gap-1.5 text-xs text-muted">
              <ShieldCheck className="size-3.5 text-emerald-500" /> 系统绝不会自动提交，最终由你亲自点击提交。
            </p>
          </div>
          ...
          {(filling || done) && a.steps.length > 0 && (
            <div className="co-rise mt-6">
              <div className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-faint">后台处理过程</div>
          ...
          {done && (
            <div className="co-rise mt-4 flex items-start gap-2.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm backdrop-blur-sm">
              ...
              <div>
                <span className="font-medium text-emerald-700 dark:text-emerald-400">真实表单已打开并完成预填。</span>{" "}
                <span className="text-muted">请仔细检查后亲自点击提交，career-ops 永远不会替你提交。</span>
              </div>
            </div>
          )}
        </div>
...
const DRIVE_VERB: Record<string, string> = { click: "已点击", type: "已输入", select: "已选择", scroll: "已滚动", "parse-error": "正在思考…", stuck: "遇到阻碍", reached_form: "已到达表单" };
  ...
  return (
    <div className="co-rise">
      <div className="flex flex-col items-center gap-3 py-7 text-center">
        ...
        <div className="font-display text-2xl text-landing">{filling ? "AI 正在填写表单…" : "正在进入申请表…"}</div>
        <p className="max-w-sm text-sm text-muted">{filling ? "AI 正在本机逐项填写真实表单；系统不会提交，你需要审核并亲自提交。" : "AI 正在本机进入真实申请表；系统不会自动提交。"}</p>
      </div>
      {last?.thumb ? (
        // eslint-disable-next-line @next/next/no-img-element
      ...
      {warns.length > 0 && (
        <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 backdrop-blur-sm">
          <div className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-amber-700 dark:text-amber-400">
            <AlertTriangle className="size-4" /> 请重点检查以下内容
          </div>
  ...
  return (
    <div className={flash ? "co-flash" : ""} style={flash ? { animationDelay: `${Math.min(index * 70, 900)}ms` } : undefined}>
      ...
      {writing ? (
        ...
        <textarea rows={3} maxLength={f.maxLength} value={value} onChange={(e) => onChange(e.target.value)} placeholder={needs ? "此项需要你填写。" : "…"} className={cn(base, "resize-none")} />
        ...
        /resume|résumé|\bcv\b|curriculum|currículum|lebenslauf/i.test(f.label || "") ? (
          <div className="flex items-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-400">
            <FileCheck2 className="size-4 shrink-0" /> 定制简历 PDF 会自动附加，你可以在真实表单中替换。
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-lg border border-dashed border-border px-3 py-2 text-sm text-muted">
            <Paperclip className="size-4 shrink-0" /> 切换到真实表单后，请手动附加此文件。
          </div>
        )
      ) : (
        <input type={["email", "tel", "url", "number", "date"].includes(f.type) ? f.type : "text"} maxLength={f.maxLength} value={value} onChange={(e) => onChange(e.target.value)} placeholder={needs ? "此项需要你填写。" : "…"} className={base} />
      )}
    </div>
  );
}
