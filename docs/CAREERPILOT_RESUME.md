...
`resume-tailor-suggest` 会读取当前策略，在岗位定制阶段返回策略公式、技术规则和逐 Fact 内容诊断。
纯技术栈但没有问题/结果语境时不会被系统补写：央国企策略要求补充问题、责任与交付 Evidence，
互联网策略要求补充挑战、所有权、方案取舍与影响，科研策略要求补充研究问题、方法、贡献与成果。
已有多分句 Fact 只允许在不增删词项的前提下生成待确认重排，接受后仍计入 30% 上限。
