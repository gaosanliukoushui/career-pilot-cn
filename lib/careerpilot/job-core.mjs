...
function jobIdentity(posting) {
  ...
  if (posting.source?.kind === 'official_url' && sourceRef) return sourceRef.toLowerCase();
...
function inferRules(text) {
  ...
  const add = (field, operator, expected, sourceQuote, severity = 'hard', confidence = 0.95) => {
    ...
    rules.push({ id, field, operator, expected, severity, explicit: true, source_quote: sourceQuote, confidence });
  };
  ...
  const locationQuote = lineContaining(text, /(?:工作地点|工作城市|招聘地点)\s*[：:]/i);
  if (locationQuote) {
    const expectedLocations = locationQuote.replace(/^.*?[：:]/, '').split(/[、，,；;]/)
      .map((item) => item.replace(/[（(].*$/, '').trim()).filter(Boolean).slice(0, 30);
    if (expectedLocations.length) add('location', 'contains_any', expectedLocations, locationQuote);
  }
...
export function inferJobPosting(rawText, source = { kind: 'pasted_text' }, hints = {}) {
  ...
  const posting = {
    ...
    source: { kind: source.kind || 'pasted_text', ...(sourceRef ? { ref: sourceRef } : {}), ...(source.file_sha256 ? { file_sha256: source.file_sha256 } : {}) },
...
export async function parseJobUrl(value, hints = {}, fetchImpl = fetch) {
  ...
  for (let redirect = 0; redirect <= 4; redirect += 1) {
    ...
    else text = cleanText(bytes.toString('utf8').replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, '\n'));
    return inferJobPosting(text, { kind: 'official_url', ref: url.href }, hints);
  }
...
export function evaluateEligibility(profile, posting) {
  const ruleResults = posting.rules.map((rule) => evaluateRule(profile, posting, rule));
  const hard = posting.rules.map((rule, index) => ({ rule, result: ruleResults[index] })).filter((item) => item.rule.severity === 'hard');
...
export function validateFitProposal(root, proposal) {
  ...
  const allowedFactIds = new Set(profile.facts.filter((fact) => fact.status === 'confirmed'
...
function sourceReliabilityCap(posting) {
  if (posting.source.kind === 'official_url') return 5;
  if (posting.source.kind === 'pdf' || posting.source.kind === 'docx') return 4;
  return 3;
}

function buildFit(profile, posting, proposals = []) {
  const confirmedFactIds = new Set(profile.facts.filter((fact) => fact.status === 'confirmed').map((fact) => fact.id));
...
export function evaluateJob(root, posting, options = {}) {
  ...
  const fit = buildFit(profile, posting, options.dimensions || []);
  ...
  const report = {
    ...
    strengths: (options.strengths || []).map(safeNarrative),
    gaps: (options.gaps || []).map(safeNarrative),
    override,
