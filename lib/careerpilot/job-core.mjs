...
function valueAfterRepeatedLabel(text, label) {
  const lines = text.split('\n').map((line) => line.trim());
  for (let index = 0; index < lines.length; index += 1) {
    if (lines[index] !== label) continue;
    let cursor = index + 1;
    while (cursor < lines.length && (!lines[cursor] || lines[cursor] === label)) cursor += 1;
...
export function inferJobPosting(rawText, source = { kind: 'pasted_text' }, hints = {}) {
  ...
  const employerName = cleanText(hints.employer?.name || employerLine.replace(/^.*?[：:]/, '') || valueAfterRepeatedLabel(text, '所属部门') || '待确认单位');
  const titleLine = lineContaining(text, /(?:岗位名称|招聘岗位|职位名称)\s*[：:]/i);
  const firstLine = text.split('\n').map((line) => line.trim()).find(Boolean) || '待确认岗位';
  const title = cleanText(hints.title || titleLine.replace(/^.*?[：:]/, '') || valueAfterRepeatedLabel(text, '职位名称') || firstLine).slice(0, 200);
  ...
  const locations = hints.locations || cleanText(locationLine.replace(/^.*?[：:]/, '') || valueAfterRepeatedLabel(text, '工作地点'))
  ...
  const posting = {
    ...
    source: {
      ...
      ...(source.capture_provider ? { capture_provider: cleanText(source.capture_provider).slice(0, 100) } : {}),
    ...
    posting_status: hints.posting_status || (deadline && deadline < new Date().toISOString().slice(0, 10)
      ? 'expired' : /申请职位/.test(text) && /职位描述/.test(text) ? 'active' : 'unknown'),
...
function normalizedCertificateCode(value) {
  return String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
}

function certificateLevel(code) {
  const match = normalizedCertificateCode(code).match(/^CET([46])$/);
  return match ? Number(match[1]) : null;
}

function certificateSatisfies(item, rule) {
  const expected = rule.expected || {};
  const expectedCode = normalizedCertificateCode(expected.kind);
  const actualCodes = [item.kind, item.level].map(normalizedCertificateCode).filter(Boolean);
  const expectedLevel = certificateLevel(expectedCode);
  const actualLevels = actualCodes.map(certificateLevel).filter(Number.isFinite);
  const sameFamilyLevel = expectedLevel != null && actualLevels.some((level) => (
    rule.operator === 'at_least' ? level >= expectedLevel : level === expectedLevel
  ));
  const exact = actualCodes.includes(expectedCode);
  const genericFamily = expectedCode === 'CET' && actualCodes.some((code) => code === 'CET' || certificateLevel(code) != null);
  if (!exact && !sameFamilyLevel && !genericFamily) return false;
  return expected.minimum_score == null || Number(item.score || 0) >= Number(expected.minimum_score);
}
...
function evaluateRule(profile, posting, rule) {
  ...
  if (rule.field === 'language_certificate') {
    const matched = candidate.find((item) => certificateSatisfies(item, rule));
    if (!matched) {
...
export function saveJobPosting(root, posting) {
  const validation = validateJobPosting(posting);
  if (!validation.valid) {
    const error = new Error('Cannot persist invalid JobPosting');
    error.code = 'JOB_POSTING_INVALID';
    error.details = validation.errors;
    throw error;
  }
...
export function loadJobRecord(root, jobId) {
  ...
  if (!existsSync(jobPath)) {
    const error = new Error(`JobPosting not found: ${jobId}`);
    error.code = 'JOB_RECORD_NOT_FOUND';
    throw error;
  }
  const posting = JSON.parse(readFileSync(jobPath, 'utf8'));
  const jobValidation = validateJobPosting(posting);
  if (!jobValidation.valid) throw new Error(`Stored JobPosting is invalid: ${jobId}`);
  if (!existsSync(matchPath)) return { posting, report: null };
  const report = JSON.parse(readFileSync(matchPath, 'utf8'));
  const reportValidation = validateMatchReport(report);
  if (!reportValidation.valid || report.job_id !== posting.id || report.job_sha256 !== sha256(stableJson(posting))) {
    throw new Error(`Stored MatchReport is invalid or stale: ${jobId}`);
  }
