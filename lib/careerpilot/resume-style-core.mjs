...
const contentStrategySchema = JSON.parse(readFileSync(join(ROOT, 'schemas', 'cn', 'resume-content-strategies.schema.json'), 'utf8'));
...
const validateContentStrategies = ajv.compile(contentStrategySchema);

if (!validateContentStrategies(contentStrategies)) {
  throw new Error(`Invalid ResumeContentStrategy catalog: ${ajv.errorsText(validateContentStrategies.errors)}`);
}
...
if (strategyIds.size !== 3 || rawCatalog.styles.some((item) => !strategyIds.has(item.content_strategy_id))) {
...
const CONTENT_STRATEGY_BY_ID = new Map(contentStrategies.strategies.map((item) => [item.id, item]));
...
export const DEFAULT_RESUME_STYLE = Object.freeze({
  ...
  font_size_pt: 9.5,
  page_margin_cm: 1,
  section_order: ['教育经历', '个人概述', '实习与工作经历', '项目经历', '校园经历', '证书与奖项', '专业技能', '基本信息'],
...
const EMPHASIS_SECTION_ORDER = Object.freeze({
  technical: ['个人概述', '实习与工作经历', '项目经历', '专业技能', '教育经历', '证书与奖项', '校园经历', '基本信息'],
  research: ['教育经历', '个人概述', '项目经历', '实习与工作经历', '证书与奖项', '校园经历', '专业技能', '基本信息'],
  campus: ['教育经历', '个人概述', '校园经历', '实习与工作经历', '项目经历', '证书与奖项', '专业技能', '基本信息'],
});
...
export function resolveResumeContentStrategy(styleOrId) {
  const id = typeof styleOrId === 'string' ? styleOrId : styleOrId?.theme;
  const strategy = CONTENT_STRATEGY_BY_ID.get(id);
  if (!strategy) {
    const error = new Error(`Unknown resume content strategy: ${id || 'missing'}`);
    error.code = 'RESUME_CONTENT_STRATEGY_UNKNOWN';
    throw error;
  }
