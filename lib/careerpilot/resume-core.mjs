        ...
        new TableCell({
          ...
          children: [
            ...
            ...(meta.length ? [new Paragraph({
              alignment: AlignmentType.LEFT,
              children: [new TextRun({ text: meta.join('  |  '), size: bodySize, color: muted, font })],
            })] : []),
          ],
        }),
        new TableCell({
...
export async function exportResume(root, variant, format, requestedPath, testHooks = {}) {
  ...
  const model = normalizedResume(root, variant);
  const expectedStatements = model.sections.flatMap((section) => section.items.map((item) => item.statement));
  const qaOptions = {
