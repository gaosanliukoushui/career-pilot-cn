...
export const DETAILED_ADDRESS_VALUE_PATTERN = /(?:(?:[\p{Script=Han}]{2,}(?:省|市|自治区|自治州|区|县|旗)){1,4}[\p{Script=Han}A-Z0-9_-]{2,}(?:路|街|大道|巷|弄|小区|社区|村|镇|乡)?|[\p{Script=Han}]{2,}(?:路|街|大道|巷|弄))\s*\d+\s*(?:号|栋|幢|单元|室|楼)/iu;
...
export function containsForbiddenResumeStatement(value) {
  const text = String(value || '');
  return FORBIDDEN_RESUME_STATEMENT_PATTERN.test(text) || DETAILED_ADDRESS_VALUE_PATTERN.test(text);
}
...
export function evaluateFactEligibility(fact, evidenceById, use = 'resume') {
  ...
  if (use === 'resume' && containsForbiddenResumeStatement(fact.statement)) reasons.push('forbidden_sensitive_content');
