...
export function generateTailoringRewriteCandidates(root, jobId, baselineVariantId) {
  ...
  const byId = new Map(profile.facts.map((fact) => [fact.id, fact]));
  const candidates = [];
  for (const factId of baseline.order) {
    const fact = byId.get(factId);
    if (!fact) continue;
    const clauses = fact.statement.split(/[；;。]+/u).map((item) => item.trim()).filter(Boolean);
    if (clauses.length < 2) continue;
    const ranked = clauses.map((statement, index) => ({ statement, index, score: jobOverlapScore(statement, posting.raw_text) }))
  ...
  return { job_id: jobId, baseline_variant_id: baselineVariantId, candidates };
}
...
export async function exportCampaignTailoredResume(root, campaignId, preview, format, requestedPath) {
  ...
  if (resumeStyle.preset === 'compact-photo' && (!confirmedVariant.sensitive_authorizations.photo || !confirmedVariant.source_photo_sha256)) {
    const error = new Error('compact-photo requires an explicitly authorized, hash-bound profile photo');
    error.code = 'PHOTO_AUTHORIZATION_REQUIRED';
    throw error;
  }
  const pageBudget = resumeStyle.preset === 'technical-two-page' ? 2 : 1;
  const result = await exportResume(root, confirmedVariant, format, requestedPath, {
    ...
    manifestExtra: {
      ...
      job_id: preview.job_id,
      target_job_title: posting.title,
      resume_style: resumeStyle.preset,
