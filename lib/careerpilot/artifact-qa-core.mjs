...
export function exactStatementSequencePresent(textItems, statements) {
  const expected = searchable((statements || []).join(''));
  if (expected.length < 4) return false;
  const items = (textItems || []).map(searchable).filter(Boolean);
  for (let start = 0; start < items.length; start += 1) {
    let block = '';
    for (let end = start; end < items.length && block.length <= expected.length; end += 1) {
      block += items[end];
      if (block === expected) return true;
      if (block.length >= expected.length) break;
    }
  }
  return false;
}
...
export async function verifyRenderedResumePdf(pdfPath, options = {}) {
  ...
  try {
    for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
      ...
      const textItems = content.items.filter((item) => 'str' in item).map((item) => item.str).filter((item) => item.trim());
  ...
  if (options.exactStatementSequences?.length
    && !options.exactStatementSequences.some((sequence) => pageItems.some((items) => exactStatementSequencePresent(items, sequence)))) {
    throw qaError('RESUME_SEMANTIC_MISMATCH', 'Rendered resume does not contain the exact confirmed Fact sequence', {
      missing_facts: options.exactStatementSequences,
    });
  }
